...
export default function InvoicePreview({ invoice, config }: Props) {
  ...
  const totalFormatted = total.toLocaleString(locale) + " " + kc
    ...
    <div id="invoice-preview" className={`invoice-a4 ${s.invoice} flex text-black bg-white`}>
      ...
      <div className={`${s["invoice__left"]} shrink-0 flex flex-col justify-between py-10 pr-5 pl-10 bg-[rgba(216,216,216,0.75)]`}>
        ...
        <div className="flex flex-col gap-4">
          ...
          <MetaField label={lang === "cs" ? "NA ÚČET"           : "ACCOUNT"}        value={account} />
          <MetaField label={lang === "cs" ? "VARIABILNÍ SYMBOL" : "VARIABLE SYMBOL"} value={vs} />
        </div>
      ...
      <div className="flex-1 flex flex-col px-10 pt-9">
          ...
          <tbody>
            {(invoice.lines ?? []).map((line, idx, arr) => (
              <tr key={line.id} style={idx < arr.length - 1 ? { borderBottom: "2px solid #D8D8D8" } : undefined}>
                ...
                <td className={`${s["invoice__td"]} pt-3 text-right`}>{line.unit_price.toLocaleString(locale)} {kc}</td>
                <td className={`${s["invoice__td"]} pt-3 text-right`}>{line.total.toLocaleString(locale)} {kc}</td>
              </tr>
            ))}
          </tbody>
          <tfoot>
            <tr>
              ...
              <td className="border-t-[3px] border-black text-right">
                <span className={`${s["invoice__total-badge"]} bg-black text-white`}>
                  {total.toLocaleString(locale)}&nbsp;{kc}
                </span>
              </td>
            </tr>
          </tfoot>
        ...
        {invoice.reverse_charge && lang !== "cs" && (
          <p className={`${s["invoice__payment-text"]} mb-3`}>{L.reverseChargeNote}</p>
        )}
...
function Party({
  ...
  role: string
  name: string
  /** e.g. "IČ: 12345678" or "VAT ID: CZ12345678" — already formatted */
  identifier?: string
  address: string
}) {
  return (
