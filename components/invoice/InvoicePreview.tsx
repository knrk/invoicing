...
export default function InvoicePreview({ invoice, config }: Props) {
  ...
  }, [total, invoiceNumber, lang, config])
    ...
    <div id="invoice-preview" className={`invoice-a4 ${s.invoice} flex text-black bg-white`}>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <MetaField
            label={lang === "cs" ? "ČÍSLO FAKTURY" : "INVOICE NUMBER"}
            value={invoiceNumber.replace(/^[A-Za-z]+/, "")}
          />
          <MetaField
            label={lang === "cs" ? "DATUM VYSTAVENÍ" : "ISSUE DATE"}
            value={invoice.issue_date ? formatDate(invoice.issue_date, lang) : ""}
          />
          <MetaField
            label={lang === "cs" ? "DATUM SPLATNOSTI" : "DUE DATE"}
            value={invoice.due_date ? formatDate(invoice.due_date, lang) : ""}
          />
          <MetaField
            label={lang === "cs" ? "PLATBA" : "PAYMENT"}
            value={invoice.payment_method ?? L.payByTransfer}
          />
        ...
        <div className="flex flex-col gap-[80px]">
          ...
          <div className="flex items-center gap-1 leading-none">
            ...
            <span className={s.invoice__thanks_text}>
      ...
      <div className="flex-1 flex flex-col px-10 pt-9">
          ...
          <Party
            ...
            address={[
              ...
              invoice.customer?.zip && invoice.customer?.city
                ...
                : "",
              lang !== "cs" ? localizeCzCountry(invoice.customer?.country ?? "", lang) : "",
            ]
              .filter(Boolean)
              .join(lang !== "cs" ? "\n" : ", ")}
          />
          ...
          <tbody>
            {(invoice.lines ?? []).map((line, idx, arr) => (
              ...
              >
                ...
                {showQtyPrice && (
                  <td className={`${s.invoice__td} pt-3 text-right`}>
                    {fmtNum(line.unit_price)} {kc}
                  </td>
                )}
                <td
        ...
        <div className="flex items-start gap-4 mb-[120px]">
          <p className={`${s.invoice__payment_text} flex-1`}>
            {getPaymentParts(lang, totalFormatted, account, vs, config.banking.constant_symbol).map(
              (part) =>
                part.bold ? (
                  <span key={part.text} className={s.invoice__payment_bold}>
                    {part.text}
                  </span>
                ) : (
                  part.text
        ...
        <div className="grid grid-cols-3 pb-10">
          <FooterCol
            label="ADRESA"
            lines={[config.supplier.street, `${config.supplier.zip} ${config.supplier.city}`]}
          />
          <FooterCol label="KONTAKT" lines={[config.supplier.phone, config.supplier.email]} />
          <FooterCol
            label="WEB"
            lines={[config.supplier.web1, config.supplier.web2].filter(Boolean) as string[]}
          />
        </div>
      </div>
    </div>
...
function MetaField({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex flex-col">
      <div className={s.invoice__meta_bar} />
      <p className={s.invoice__meta_label}>{label}</p>
      <p className={s.invoice__meta_value}>{value || "—"}</p>
    </div>
  )
}
  ...
  return (
    <div className="flex-1 flex flex-col gap-1">
      <div className="flex items-center gap-3">
        <div className={`${s.invoice__party_bar} bg-black shrink-0`} />
        <p className={s.invoice__party_role}>{role}</p>
      </div>
...
function localizeCzCountry(country: string, lang: string): string {
  ...
  if (country === "CZ" || country === "Česká republika" || country === "Czech Republic")
...
function FooterCol({ label, lines }: { label: string; lines: string[] }) {
  return (
    <div>
      <p className={s.invoice__footer_label}>{label}</p>
      {lines.filter(Boolean).map((line) => (
        <p key={line} className={s.invoice__footer_line}>
          {line}
        </p>
      ))}
    </div>
